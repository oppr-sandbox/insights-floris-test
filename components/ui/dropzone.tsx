  ...
  ({ className, children, ...props }, ref) => {
    ...
    return (
      <label
        ...
        {...props}
        className={cn(
          "cursor-pointer rounded-sm bg-secondary px-4 py-2 font-medium ring-offset-background transition-colors focus-within:outline-none hover:bg-secondary/20 has-[input:focus-visible]:ring-2 has-[input:focus-visible]:ring-ring has-[input:focus-visible]:ring-offset-2",
          className
        )}
      >
        {children}
...
interface CaptureDropzoneTriggerProps
  extends React.LabelHTMLAttributes<HTMLLabelElement> {}

const CaptureDropzoneTrigger = forwardRef<HTMLLabelElement, CaptureDropzoneTriggerProps>(
  ({ className, children, ...props }, ref) => {
    const context = useDropzoneContext();
    if (!context) {
      throw new Error("CaptureDropzoneTrigger must be used within a Dropzone");
    }
...
CaptureDropzoneTrigger.displayName = "CaptureDropzoneTrigger";
